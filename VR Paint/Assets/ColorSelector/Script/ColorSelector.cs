...
public class ColorSelector : MonoBehaviour {
	...
	void OnDisable() {
		...
		if (device.GetTouch(SteamVR_Controller.ButtonMask.Trigger)) {
			Ray vrRay = new Ray(rightController.transform.position, rightController.transform.forward);
			RaycastHit hit = new RaycastHit();
			if (Physics.Raycast(vrRay, out hit)) {
	...
	void SelectInnerColor(Vector2 delta){
		...
		if (v >= 0.15f && w >= -0.15f && u >= -0.15f) {
			...
			innerDelta = delta;
			rightController.GetComponent<SteamVR_LaserPointer>().color = finalColor;
